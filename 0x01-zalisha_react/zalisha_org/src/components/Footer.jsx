  ...
  <section className={`${styles.flexCenter} ${styles.paddingY} flex-col`}>
    ...
    <div className={`${styles.flexStart} md:flex-row flex-col mb-8 w-full`}>
      <div className={`flex-1 flex flex-col justify-start mr-10`}>
        ...
        <p className={`${styles.paragraph} mt-4 max-w-[290px]`}>
    ...
    <div className="w-full flex justify-between items-center md:flex-row flox-col pt-6 border-t-[1px] border-t-[#3f3e45]">
      <p className="font-poppins font-normal text-center text-[18px] leading-[27px] text-white">
        2022 Zalisha. All rights reserved.
      </p>
      <div className="flex flex-row mt-6 md:mt-0">
        {socialMedia.map((social, index) => (
          <img src={social.icon} key={social.id} alt={social.id} className={`w-[26px] h-[26px] object-contain cursor-pointer ${index !== socialMedia.length ? 'mr-6': 'mr-0'}`}/>
        ))}
      </div>
    </div>
